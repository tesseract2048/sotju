 ...
 */
public class EwebNewsListRule extends Rule {
    ...
    private final static String PAGE_PATTERN = "<input type=\"hidden\" name=\"curPage\" value=\"(?<page>\\d+)\">";

    public EwebNewsListRule() {
        setId("eweb_news_list");
        setSiteId("eweb");
        setUrlPattern("http://e\\.tju\\.edu\\.cn/News/([a-zA-Z]+)List\\.do");
        ...
        setExtractors(Arrays
                .asList(new Extractor[] {
                    ...
                    new Extractor(PatternType.REGEX, PAGE_PATTERN).function(
                            "page",
                            new FunctionInvokeChain().append(
                                    FunctionType.ABSOLUTE_URL, "$url",
                                    "?todo=nextPage&curPage=$ret").append(
                                    FunctionType.FETCH)),
